...
const { embedMessageReplies, usesRawReplies } = require('../utils/embedReplies');
...
module.exports = {
  ...
  async execute(message, client) {
    ...
    if (command) {
      // Roleplay and image commands keep raw replies so their media renders
      // inline; everything else answers in an embed.
      const target = usesRawReplies(command) ? message : embedMessageReplies(message);

      if (!usable && !PREMIUM_EXEMPT_PREFIX_COMMANDS.has(commandName)) {
        return embedMessageReplies(message).reply(NO_LICENSE_MESSAGE).catch(() => {});
      }
      if (command.permissions && !hasCommandPermissions(message.member, command.permissions)) {
        return embedMessageReplies(message).reply("You don't have permission to use that command.").catch(() => {});
      }
      try {
        await command.execute(target, args, config, client);
      } catch (error) {
        console.error(`Error executing prefix command ${commandName}:`, error);
        await embedMessageReplies(message).reply('Something went wrong while running that command.').catch(() => {});
      }
      return;
    }
    ...
    if (response) await embedMessageReplies(message).channel.send(response).catch(() => {});
  },
};
