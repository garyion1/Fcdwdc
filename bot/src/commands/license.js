...
const { getAdminGuildId, createLicense, getLicense, revokeLicense, listLicenses } = require('../utils/licenses');
  ...
  async execute(interaction) {
    ...
    if (sub === 'revoke') {
      ...
      if (result.guildId) {
        ...
        config.premium = { active: false, tier: null, licenseKey: null, expiresAt: null };
    ...
    if (sub === 'list') {
      ...
      const lines = licenses
        .slice(0, 25)
        .map((l) => `\`${l.key}\` — ${l.tier} — ${l.redeemed ? `redeemed (guild ${l.guildId})` : 'unredeemed'}`)
        .join('\n');
      const embed = baseEmbed()
