  ...
  async execute(interaction) {
    ...
    if (sub === 'add') {
      ...
      if (interaction.client.prefixCommands.has(name)) {
        return interaction.reply({ content: `\`${name}\` is already a built-in command name. Pick a different name.` });
      }
    ...
    if (sub === 'remove') {
      ...
      if (!config.customCommands[name]) {
        return interaction.reply({ content: `No custom command named \`${name}\` exists.` });
      }
      delete config.customCommands[name];
      saveConfig(interaction.guildId);
      return interaction.reply({ embeds: [baseEmbed(COLORS.success).setDescription(`Custom command \`${name}\` removed.`)] });
    }

    if (sub === 'list') {
      const names = Object.keys(config.customCommands);
      if (names.length === 0) {
        return interaction.reply({ content: 'No custom commands have been created yet.' });
      }
      return interaction.reply({
